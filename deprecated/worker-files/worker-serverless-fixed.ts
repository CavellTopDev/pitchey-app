/**
 * Production Worker with Neon Serverless Driver - Fixed Column Names
 * Uses correct camelCase column names from schema
 */
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    try {
          ...
          .where(
            and(
              ...
              or(
                ...
                like(schema.pitches.longSynopsis, `%${query}%`)
              )
            )
          )
          .limit(20);
        ...
        const result = await withCache(
          ...
          async () => {
            const [pitchCount, viewStats, saveStats, ndaCount] = await Promise.all([
              globalDb.select({ count: count() })
                .from(schema.pitches)
                .where(eq(schema.pitches.userId, userPayload.id)),
      ...
      // Production Dashboard (simplified without missing tables)
      if (path === '/api/production/dashboard' && method === 'GET') {
        ...
        const result = await withCache(
          ...
          60,
          async () => {
            // For now, return placeholder data until production tables are created
            const [pitchCount] = await Promise.all([
              globalDb.select({ count: count() })
                .from(schema.pitches)
                .where(eq(schema.pitches.status, 'published'))
            ]);
            ...
            return {
              ...
              stats: {
                ...
                budgetAllocated: 0,
                teamSize: 0,
                submissions: pitchCount[0]?.count || 0,
              },
              activeProjects: [],
              lastUpdated: new Date().toISOString()
            };
      ...
      if (path === '/api/saved-pitches' && method === 'POST') {
        ...
        if (existing.length > 0) {
          ...
          await globalDb
            .update(schema.pitches)
            .set({ saveCount: sql`GREATEST(COALESCE(save_count, 0) - 1, 0)` })
          ...
          await globalDb
            .update(schema.pitches)
            .set({ saveCount: sql`COALESCE(save_count, 0) + 1` })
            .where(eq(schema.pitches.id, pitch_id));
