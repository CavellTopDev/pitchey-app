...
export default defineConfig({
  ...
  build: {
    ...
    minify: 'esbuild', // Use esbuild instead of terser to avoid hoisting issues
    esbuildOptions: {
      drop: process.env.NODE_ENV === 'production' ? ['console', 'debugger'] : [],
      legalComments: 'none',
      // Keep variable names to avoid hoisting issues
      keepNames: true,
    },
    commonjsOptions: {
    ...
    rollupOptions: {
      output: {
        ...
        manualChunks: (id) => {
          ...
          // Include use-sync-external-store in vendor chunk
